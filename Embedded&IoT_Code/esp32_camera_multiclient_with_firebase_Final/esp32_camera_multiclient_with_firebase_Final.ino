...
#define API_KEY "AIzaSyBgjLyqpF-Z-S59aPKfbRg7EVGdeNS3TGU"  // Firebase project API Key
#define USER_EMAIL "Nourhanelnagar3@gmail.com"      // Authorized Email and 
#define USER_PASSWORD "ASHNHomeX"           // Password

#define DATABASE_URL "https://homex-fd825-default-rtdb.firebaseio.com/"   //real time database

#define STORAGE_BUCKET_ID "homex-fd825.appspot.com"   //Firebase storage
...
void setup()
{
  //  // Serial port for debugging purposes (Setup Serial connection:)
  Serial.begin(115200);
...
void loop() {
  ...
  if (Firebase.RTDB.getString(&fbdo, "/esp1/ultrasonic/state")) {
    String ultrasonicState = fbdo.stringData(); // Retrieve the actual string data
  ...
  if (Firebase.ready()){
    ...
    if (Firebase.Storage.upload(&fbdo, STORAGE_BUCKET_ID /* Firebase Storage bucket id */, FILE_PHOTO_PATH /* path to local file */, mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */, BUCKET_PHOTO /* path of remote file stored in the bucket */, "image/jpeg" /* mime type */,fcsUploadCallback)){
      ...
      // Perform actions if the value is not "0"
      Serial.println("Value is 0, no action taken.");
    }
  }
}
